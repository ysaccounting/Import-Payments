...
def parse_ticket_evolution(file, date_start=None, date_end=None) -> pd.DataFrame:
    ...
    # Detect xlsx vs csv by trying to read as Excel first
    try:
    ...
    if date_start or date_end:
        ...
        if date_col:
            # Handle both datetime objects (xlsx) and strings (csv)
            raw_dates = df[date_col]
            if pd.api.types.is_datetime64_any_dtype(raw_dates) or hasattr(raw_dates.iloc[0] if len(raw_dates) > 0 else None, 'date'):
                df["_date"] = pd.to_datetime(raw_dates, errors="coerce").dt.date
            else:
                df["_date"] = pd.to_datetime(
                    raw_dates.astype(str).str.strip().str.strip('"'),
                    format="mixed",
                    errors="coerce"
                ).dt.date
            if date_start:
